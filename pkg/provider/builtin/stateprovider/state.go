...
func (p *StateProvider) handleMissing(key string, inputs map[string]any) (*provider.Output, error) {
	...
	// If an explicit fallback is provided, use it (even if the value is nil).
	// This lets the resolver chain stop here with the fallback value.
	fallback, hasFallback := inputs["fallback"]
	if hasFallback {
		return &provider.Output{
			Data: fallback,
		}, nil
	...
	// No fallback specified -- return an error so the resolver fallback chain
	// can continue to the next source (used with onError: continue).
	return nil, fmt.Errorf("state key %q not found (no fallback): %w", key, state.ErrKeyNotFound)
}
